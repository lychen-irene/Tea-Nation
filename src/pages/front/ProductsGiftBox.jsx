...
const ProductsTeaCan = () => {
  const [teaGiftBox, setTeaGiftBox] = useState([]);
      ...
      try {
        ...
        if (res.data.success) {
          ...
          const filteredCans = allProducts.filter(
            (item) => item.category === "茶禮盒",
          );

          setTeaGiftBox(filteredCans);
        }
      } catch (error) {
    ...
    return (
      ...
      >
        <div
          ...
          role="status"
        ></div>
        <span className="ms-3 text-secondary">茶葉罐系列載入中...</span>
      </div>
    );
  }
          ...
          {teaGiftBox.length > 0 ? (
            teaGiftBox.map((tea) => (
              <div key={tea.id} className="col-12 col-sm-6 col-lg-3">
                {/* 💡 關鍵跳轉：點擊卡片任何地方，都會把 ID 帶到 SingleProduct 詳細頁面 */}
                <Link
                  to={`/product/${tea.id}`}
                  ...
                  <div className="product-card h-100">
                    ...
                    <div className="d-flex justify-content-between align-items-end px-1">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.preventDefault(); // 阻止 Link 跳轉，只執行加入購物車
                          addToCart(tea.id);
                        }}
                      >
                        <img
